...
function formulaires_login_terminal_charger_dist($cible="",$login="",$prive=null)
{
	...
	if (is_null($prive) ? is_url_prive22($cible) : $prive) {
...
function formulaires_login_terminal_verifier_dist($cible="",$login="",$prive=null){
	...
	if (!is_array($auteur)) {
		...
		if (strlen($session_password))
			$erreurs['password'] = _T('forum_erreur_pass');
	...
	return (is_null($prive) ? is_url_prive22($cible) : $prive)
	?  login_autoriser2() : array();
}

function login_autoriser2()
{
	include_spip('inc/autoriser');
	if (!autoriser('ecrire')){
...
function formulaires_login_terminal_traiter_dist($cible="",$login="",$prive=null){
	$res = array();
	
	$res['redirect'] = parametre_url(generer_url_public('terminal','',false),'return',_request('return'));
	return $res;
}
