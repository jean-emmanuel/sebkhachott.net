...
function formulaires_editer_auteur_public_charger_dist($id_auteur='new', $retour='', $associer_objet='', $config_fonc='auteurs_edit_config2', $row=array(), $hidden=''){
 ...
 */
function formulaires_editer_auteur_public_identifier_dist($id_auteur='new', $retour='', $associer_objet='', $config_fonc='auteurs_edit_config2', $row=array(), $hidden=''){
...
function auteurs_edit_config2($row)
{
	global $spip_lang;
...
function formulaires_editer_auteur_public_verifier_dist($id_auteur='new', $retour='', $associer_objet='', $config_fonc='auteurs_edit_config2', $row=array(), $hidden=''){
	...
	if ($email = _request('email')){
		...
		if (!autoriser('modifier','auteur',$id_auteur,null,array('email'=>'?'))
			...
			$erreurs['email'] = (($id_auteur==$GLOBALS['visiteur_session']['id_auteur'])?_T('forum_email_non_valide'):_T('form_prop_indiquer_email'));
		}
		if (!strlen(trim($email))) {
		    $erreurs['email'] = _T('forum_email_non_valide');
		}
	...
	if ($err = auth_verifier_login($auth_methode, _request('new_login'), $id_auteur)){
		...
		if ($p = _request('new_pass')) {
			if ($p != _request('new_pass2')) {
				$erreurs['new_pass'] = _T('forum_nouveau_pass_identiques');
				$erreurs['message_erreur'] .= _T('forum_nouveau_pass_identiques');
			}
...
// http://doc.spip.org/@inc_editer_mot_dist
function formulaires_editer_auteur_public_traiter_dist($id_auteur='new', $retour='', $associer_objet='', $config_fonc='auteurs_edit_config2', $row=array(), $hidden=''){
	...
	if (!autoriser('modifier','auteur',$id_auteur,null,array('email'=>'?'))){
		...
		if ($GLOBALS['visiteur_session']['id_auteur']==$id_auteur
		    AND (strlen(trim($email_nouveau)))
			AND $email_nouveau!=($email_ancien=sql_getfetsel('email', 'spip_auteurs', 'id_auteur='.intval($id_auteur)))){
			$envoyer_mail = charger_fonction('envoyer_mail','inc');
			$texte = _T('forum_nouveau_mail_confirmation_corps',
											array('signature'=>_T('forum_mail_signature'),'url'=>generer_action_auteur('confirmer_email', $email_nouveau,parametre_url($retour, 'email_modif','ok'))));
			$envoyer_mail($email_nouveau,'[' . $GLOBALS['meta']['nom_site'] . "] " . _T('forum_nouveau_mail_confirmation_sujet'),$texte);
			set_request('email_confirm',$email_nouveau);
			if ($email_ancien)
				$envoyer_mail($email_ancien,'[' .$GLOBALS['meta']['nom_site'] . "] " . _T('forum_nouveau_mail_confirmation_sujet'),_T('forum_nouveau_mail_notification_corps',array('signature'=>_T('forum_mail_signature'),'email'=>$email_nouveau)));
	...
	if (isset($email_change) AND strlen($res['message_ok'])) {
	    return _T('forum_nouveau_mail_notifie');
	}
